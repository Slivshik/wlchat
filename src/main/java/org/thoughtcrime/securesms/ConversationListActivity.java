...
public class ConversationListActivity extends PassphraseRequiredActionBarActivity
  ...
  public void openConversation(int chatId, int startingPosition) {
    ...
    if (fwdAccId == dcContext.getAccountId() && dcContext.getChat(chatId).isSelfTalk()) {
      ...
      // Shared element transition for smooth animation
      android.view.View avatarView = findCurrentAvatarView();
      if (avatarView != null) {
        android.util.Pair<android.view.View, String> avatarTransition =
            android.util.Pair.create(avatarView, "chat_avatar");
        android.app.ActivityOptions options =
            android.app.ActivityOptions.makeSceneTransitionAnimation(this, avatarTransition);
  ...
  private android.view.View findCurrentAvatarView() {
    try {
      RecyclerView recyclerView = findViewById(R.id.list);
      if (recyclerView != null) {
        // Find the first visible item's avatar
        LinearLayoutManager lm = (LinearLayoutManager) recyclerView.getLayoutManager();
        if (lm != null) {
          int firstVisible = lm.findFirstVisibleItemPosition();
          if (firstVisible >= 0) {
            View holder = recyclerView.getChildAt(0);
            if (holder != null) {
              return holder.findViewById(R.id.avatar);
            }
          }
        }
      }
