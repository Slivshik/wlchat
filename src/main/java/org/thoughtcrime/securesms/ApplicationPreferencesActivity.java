...
public class ApplicationPreferencesActivity extends PassphraseRequiredActionBarActivity
  ...
  private static final String PREFERENCE_CATEGORY_RELAYS = "preference_category_relays";
  ...
  public static class ApplicationPreferenceFragment extends CorrectedPreferenceFragment
    ...
    public void onCreate(Bundle icicle) {
      ...
      this.findPreference(PREFERENCE_CATEGORY_RELAYS)
          .setOnPreferenceClickListener(new RelayCategoryClickListener());
      ...
      this.findPreference(PREFERENCE_CATEGORY_CONNECTIVITY)
          .setSummary(
              DcHelper.getConnectivitySummary(
                  getActivity(), getString(R.string.connectivity_connected)));
      this.findPreference(PREFERENCE_CATEGORY_RELAYS)
          .setSummary(getRelaySummary());
      this.findPreference(PREFERENCE_CATEGORY_HELP)
          .setSummary(AdvancedPreferenceFragment.getVersion(getActivity()));
    }

    private String getRelaySummary() {
      try {
        chat.delta.rpc.Rpc rpc = DcHelper.getRpc(getActivity());
        int accId = DcHelper.getContext(getActivity()).getAccountId();
        java.util.List<chat.delta.rpc.types.TransportListEntry> relays = rpc.listTransportsEx(accId);
        if (relays.isEmpty()) {
          return getString(R.string.email_relays_summary);
        }
        String mainAddr = rpc.getConfig(accId, DcHelper.CONFIG_CONFIGURED_ADDRESS);
        int count = relays.size();
        return count + " relay" + (count != 1 ? "s" : "") + " · " + getString(R.string.email_relays_summary);
      } catch (Exception e) {
        return getString(R.string.email_relays_summary);
      }
    }
    ...
    private class RelayCategoryClickListener implements Preference.OnPreferenceClickListener {
      @Override
      public boolean onPreferenceClick(Preference preference) {
        boolean result =
            ScreenLockUtil.applyScreenLock(
                getActivity(),
                getString(R.string.email_relays),
                getString(R.string.enter_system_secret_to_continue),
                screenLockLauncher);
        if (!result) {
          Intent intent = new Intent(getActivity(), org.thoughtcrime.securesms.relay.RelayListActivity.class);
          startActivity(intent);
        }
        return true;
      }
