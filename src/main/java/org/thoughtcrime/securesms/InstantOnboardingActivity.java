...
public class InstantOnboardingActivity extends BaseActionBarActivity
  ...
  private static final String DEFAULT_CHATMAIL_HOST = "chatmail.at";
  ...
  private void initializeResources() {
    ...
    // WL CHAT auto-registration
    Button wlSignupButton = findViewById(R.id.wl_signup_button);
    wlSignupButton.setOnClickListener(v -> startWlChatRegistration());
  }
  ...
  private void startWlChatRegistration() {
    EditText emailInput = findViewById(R.id.wl_email_text);
    EditText passwordInput = findViewById(R.id.wl_password_text);
    ...
    if (!email.endsWith("@wlrus.lol")) {
      Toast.makeText(this, "Only @wlrus.lol emails are supported for WL CHAT registration", Toast.LENGTH_LONG).show();
      return;
    }
    ...
    if (avatarChanged) {
      try {
        AvatarHelper.setSelfAvatar(this, avatarBmp);
    ...
    // Auto-configure with wlrus.lol IMAP/SMTP settings
    if (progressDialog != null) {
      progressDialog.dismiss();
      progressDialog = null;
    }
    ...
    progressDialog.setMessage("Setting up WL CHAT...");
    ...
    new Thread(
            () -> {
              ...
              try {
                ...
                param.imapServer = "imap.masterhost.ru";
                param.imapPort = 993;
                param.imapSecurity = chat.delta.rpc.types.Socket.ssl;
                param.imapUser = email;
                param.smtpServer = "smtp.masterhost.ru";
                param.smtpPort = 465;
                param.smtpSecurity = chat.delta.rpc.types.Socket.ssl;
                param.smtpUser = email;
                param.smtpPassword = password;

                rpc.addOrUpdateTransport(dcContext.getAccountId(), param);
